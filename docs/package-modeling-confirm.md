# Package 建模功能点 — 实施计划（v1.4 完成版）

**版本**: 1.4
**日期**: 2026-09-26
**状态**: ✅ 实施完成 + 全量测试通过（后端 8 packages ok，前端根 183 / frontend 218）+ Docker 部署完成

---

## v1.4 修订要点（用户反馈）

1. **树根节点**：从合成的 `project` 节点改为「与项目同名的顶级 Package」（即项目自动建的根 Package）
   - 工程树根 = 自动建的根 Package
   - 根 Package 不可"移动到顶级" / 不可"删除"（防止项目丢失默认工作区）
2. **嵌套语义修正**：能否嵌套取决于**目标节点**（是否有 body），而不是被拖元素
   - 拖 usage 到 def → ✅ 允许（之前错误拒绝）
   - 拖 def 到 usage → ❌ 拒绝（usage 无 body）
   - 拖 usage 到 usage → ❌ 拒绝
